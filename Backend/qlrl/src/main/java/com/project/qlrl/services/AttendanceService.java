        ...
        try {
            for (int i = 0; i < lstAbsent.size() ; i++) {
                ...
                for (int j = 0; j < listAttendance.size(); j++) {
                    if(listAttendance.get(j).get("USER_ID").toString().equals(lstAbsent.get(i).get("Ma_sv"))){
                        isAdd = false;
                    }
                }
                if(isAdd == true){
                    lstAbsent.get(i).put("userId", lstAbsent.get(i).get("Ma_sv").toString());
                    lstAbsent.get(i).put("lesson", param.get("lesson").toString());
                    lstAbsent.get(i).put("idClass", param.get("idClass").toString());
                    lstAbsent.get(i).put("idSubject", param.get("idSubject").toString());
